...
export const VolSignUpPage = () => {
  ...
  useEffect(() => {
    counter > 0 ? setIsDisabled(false) : setIsDisabled(true)
  }, [counter]);
        ...
        localData.map((bible) => {
          if (bible.key === book.key) {
            if (bible.isPickup) {
              ...
              return { ...bible, isPickup: false };
            }
            if (counter > 1 ) {
              setShowModal(true);
            } else {
              setCounter((pre) => pre + 1);
              return { ...bible, isPickup: true };
            }
          }
          return bible;
        })
  ...
  return (
    <div className="flex flex-col ">
      ...
      <p className="text-red-500 text-center py-5 text-[14px] md:text-2xl ">
        If the book is{" "}
        <span className=" line-through text-gray-400">grayed out</span>, someone
        had picked up!
      </p>
        ...
        <div className="grid px-5 grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-3 mt-5">
          {localData.map((example, i) => (
            <Checkbox
              ...
              book={example}
              bookid={i}
              onChange={() => handleOnChange(example)}
            />
          ))}
        </div>
      ...
      {showModal && (
        <Modal setShowModal={handleShowModal} checkedItems={localData} />
      )}
    </div>
  );
};
